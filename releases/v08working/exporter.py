...
def export_to_excel(results: list, vlan: str = "UNKNOWN") -> str:
    ...
    Параметры:
        results  — список кортежей (ip, hostname, ping, status)
        vlan     — название VLAN для имени файла (например '150' или 'ALL')
    ...
    # ── заголовки ──────────────────────────────────────────────────────────────
    headers = ["IP", "Hostname", "Ping", "Status"]
    ws.append(headers)
    ...
    for row_data in results:
        ...
        row_idx = ws.max_row
        is_online = str(row_data[3]).upper() == "ONLINE"
    ...
    # ── авто-фильтр ───────────────────────────────────────────────────────────
    ws.auto_filter.ref = f"A1:D{ws.max_row - 1}"
